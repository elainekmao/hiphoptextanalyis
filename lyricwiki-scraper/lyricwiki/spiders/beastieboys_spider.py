...
class LyricWikiSpider(CrawlSpider):
    name = "beastieboys"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Beastie_Boys",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Beastie_Boys:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
