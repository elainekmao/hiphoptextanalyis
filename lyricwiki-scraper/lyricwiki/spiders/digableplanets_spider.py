...
class LyricWikiSpider(CrawlSpider):
    name = "digableplanets"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Digable_Planets",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Digable_Planets:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
