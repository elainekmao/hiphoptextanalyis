...
class LyricWikiSpider(CrawlSpider):
    name = "tooshort"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Too_$hort",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Too_.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
