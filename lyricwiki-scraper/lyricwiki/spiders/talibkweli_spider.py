...
class LyricWikiSpider(CrawlSpider):
    name = "talibkweli"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Talib_Kweli",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Talib_Kweli:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
