...
class LyricWikiSpider(CrawlSpider):
    name = "ludacris"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Ludacris",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Ludacris:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
