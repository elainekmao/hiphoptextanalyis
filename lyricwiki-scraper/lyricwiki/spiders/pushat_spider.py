...
class LyricWikiSpider(CrawlSpider):
    name = "pushat"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Pusha_T",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Pusha_T:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
