...
class LyricWikiSpider(CrawlSpider):
    name = "g-unit"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/G-Unit",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/G-Unit:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
