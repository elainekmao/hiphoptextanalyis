...
class LyricWikiSpider(CrawlSpider):
    name = "kanye"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Kanye_West",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Kanye_West:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
