...
class LyricWikiSpider(CrawlSpider):
    name = "asaprocky"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/ASAP_Rocky",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/ASAP_Rocky:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
