...
class LyricWikiSpider(CrawlSpider):
    name = "chamillionaire"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Chamillionaire",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Chamillionaire:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
