...
class LyricWikiSpider(CrawlSpider):
    name = "doggpound"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Tha_Dogg_Pound",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Tha_Dogg_Pound:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
