...
class LyricWikiSpider(CrawlSpider):
    name = "macklemore"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Macklemore",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Macklemore.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
