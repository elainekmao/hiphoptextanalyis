...
class LyricWikiSpider(CrawlSpider):
    name = "grandmasterflash"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Grandmaster_Flash",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Grandmaster_Flash:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
