...
class LyricWikiSpider(CrawlSpider):
    name = "maryj"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Mary_J._Blige",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Mary_J.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
