...
class LyricWikiSpider(CrawlSpider):
    name = "naughty"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Naughty_By_Nature",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Naughty_By_Nature:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
