...
class LyricWikiSpider(CrawlSpider):
    name = "ghostfacekillah"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Ghostface_Killah",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Ghostface_Killah:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
