...
class LyricWikiSpider(CrawlSpider):
    name = "mystikal"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Mystikal",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Mystikal:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
