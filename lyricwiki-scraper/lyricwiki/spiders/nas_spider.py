...
class LyricWikiSpider(CrawlSpider):
    name = "nas"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Nas-05-One_Mic-Rns",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Nas:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
