...
class LyricWikiSpider(CrawlSpider):
    name = "2chainz"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/2_Chainz",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/2_Chainz:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
