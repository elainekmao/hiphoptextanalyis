...
class LyricWikiSpider(CrawlSpider):
    name = "chingy"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Chingy",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Chingy:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
