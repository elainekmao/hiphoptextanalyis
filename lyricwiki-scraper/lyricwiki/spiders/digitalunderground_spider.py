...
class LyricWikiSpider(CrawlSpider):
    name = "digitalunderground"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Digital_Underground",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Digital_Underground:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
