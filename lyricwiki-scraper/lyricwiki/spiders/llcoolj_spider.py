...
class LyricWikiSpider(CrawlSpider):
    name = "llcoolj"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/LL_Cool_J",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/LL_Cool_J:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
