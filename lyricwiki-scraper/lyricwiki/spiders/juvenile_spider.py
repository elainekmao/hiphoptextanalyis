...
class LyricWikiSpider(CrawlSpider):
    name = "juvenile"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Juvenile",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Juvenile:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
