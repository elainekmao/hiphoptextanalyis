...
class LyricWikiSpider(CrawlSpider):
    name = "timbaland"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Timbaland",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Timbaland:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
