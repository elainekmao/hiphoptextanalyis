...
class LyricWikiSpider(CrawlSpider):
    name = "clipse"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Clipse",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Clipse:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
