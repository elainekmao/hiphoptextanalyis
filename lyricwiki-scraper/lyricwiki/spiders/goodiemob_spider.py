...
class LyricWikiSpider(CrawlSpider):
    name = "goodiemob"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Goodie_Mob",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Goodie_Mob:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
