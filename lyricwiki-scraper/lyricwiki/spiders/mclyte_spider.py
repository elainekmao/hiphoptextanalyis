...
class LyricWikiSpider(CrawlSpider):
    name = "mclyte"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/MC_Lyte",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/MC_Lyte:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
