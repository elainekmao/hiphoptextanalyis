...
class LyricWikiSpider(CrawlSpider):
    name = "madvillain"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Madvillain",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Madvillain:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
