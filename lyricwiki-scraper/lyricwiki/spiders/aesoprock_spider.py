...
class LyricWikiSpider(CrawlSpider):
    name = "aesoprock"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Aesop_Rock",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Aesop_Rock.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
