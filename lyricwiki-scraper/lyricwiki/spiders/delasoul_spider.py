...
class LyricWikiSpider(CrawlSpider):
    name = "delasoul"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/De_La_Soul",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/De_La_Soul:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
