...
class LyricWikiSpider(CrawlSpider):
    name = "snoop"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Snoop_Doggy_Dog",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Snoop.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
