...
class LyricWikiSpider(CrawlSpider):
    name = "getoboys"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Geto_Boys",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Geto_Boys:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
