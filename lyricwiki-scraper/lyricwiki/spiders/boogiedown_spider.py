...
class LyricWikiSpider(CrawlSpider):
    name = "boogiedown"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Boogie_Down_Productions",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Boogie_Down_Productions:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
