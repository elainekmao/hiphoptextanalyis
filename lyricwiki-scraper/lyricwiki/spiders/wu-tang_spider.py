...
class LyricWikiSpider(CrawlSpider):
    name = "wu-tang"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Wu-Tang_Clan",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Wu-Tang_Clan:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
