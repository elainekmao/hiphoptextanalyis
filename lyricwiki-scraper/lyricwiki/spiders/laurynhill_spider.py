...
class LyricWikiSpider(CrawlSpider):
    name = "laurynhill"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Lauryn_Hill",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Lauryn_Hill:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
