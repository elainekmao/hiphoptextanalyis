...
class LyricWikiSpider(CrawlSpider):
    name = "domogenesis"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Domo_Genesis",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Domo_Genesis:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
