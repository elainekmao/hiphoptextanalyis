...
class LyricWikiSpider(CrawlSpider):
    name = "thediplomats"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/The_Diplomats",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/The_Diplomats:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
