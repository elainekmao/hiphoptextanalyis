...
class LyricWikiSpider(CrawlSpider):
    name = "gangstarr"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Gang_Starr",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Gang_Starr:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
