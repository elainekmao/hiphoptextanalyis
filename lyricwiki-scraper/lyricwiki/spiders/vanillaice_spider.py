...
class LyricWikiSpider(CrawlSpider):
    name = "vanillaice"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Vanilla_Ice",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Vanilla_Ice:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
