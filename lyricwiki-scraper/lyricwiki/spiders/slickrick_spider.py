...
class LyricWikiSpider(CrawlSpider):
    name = "slickrick"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Slick_Rick",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Slick_Rick:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
