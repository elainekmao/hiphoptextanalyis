...
class LyricWikiSpider(CrawlSpider):
    name = "lupefiasco"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Lupe_Fiasco",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Lup.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
