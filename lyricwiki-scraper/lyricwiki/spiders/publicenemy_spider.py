...
class LyricWikiSpider(CrawlSpider):
    name = "publicenemy"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Public_Enemy",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Public_Enemy:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
