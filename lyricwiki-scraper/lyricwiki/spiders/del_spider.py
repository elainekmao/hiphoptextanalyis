...
class LyricWikiSpider(CrawlSpider):
    name = "del"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Del_Tha_Funkee_Homosapien",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Del_Tha_Funkee_Homosapien:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
