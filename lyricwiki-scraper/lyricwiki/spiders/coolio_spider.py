...
class LyricWikiSpider(CrawlSpider):
    name = "coolio"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Coolio",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Coolio:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
