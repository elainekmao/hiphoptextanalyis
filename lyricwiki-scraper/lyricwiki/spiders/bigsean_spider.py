...
class LyricWikiSpider(CrawlSpider):
    name = "bigsean"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Big_Sean",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Big_Sean:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
