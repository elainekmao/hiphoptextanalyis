...
class LyricWikiSpider(CrawlSpider):
    name = "wyclef"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Wyclef_Jean",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Wyclef_Jean:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
