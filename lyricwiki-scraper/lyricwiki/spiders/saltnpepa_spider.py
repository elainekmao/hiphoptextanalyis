...
class LyricWikiSpider(CrawlSpider):
    name = "saltnpepa"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Salt-N-Pepa",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Salt-N-Pepa:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
