...
class LyricWikiSpider(CrawlSpider):
    name = "masterp"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Master_P",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Master_P:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
