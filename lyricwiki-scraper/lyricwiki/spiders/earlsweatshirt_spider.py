...
class LyricWikiSpider(CrawlSpider):
    name = "earlsweatshirt"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Earl_Sweatshirt",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Earl_Sweatshirt:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
