...
class LyricWikiSpider(CrawlSpider):
    name = "lilwayne"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Lil_Wayne",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Lil_Wayne:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
