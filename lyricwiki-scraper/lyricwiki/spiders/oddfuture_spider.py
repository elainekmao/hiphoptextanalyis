...
class LyricWikiSpider(CrawlSpider):
    name = "oddfuture"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Odd_Future",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Odd_Future:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
