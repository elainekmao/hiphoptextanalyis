...
class LyricWikiSpider(CrawlSpider):
    name = "arrested"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Arrested_Development",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Arrested_Development:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
