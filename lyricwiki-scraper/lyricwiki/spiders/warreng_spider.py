...
class LyricWikiSpider(CrawlSpider):
    name = "warreng"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Warren_G",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Warren_G:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
