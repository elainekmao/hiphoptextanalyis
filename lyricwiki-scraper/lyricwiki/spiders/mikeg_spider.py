...
class LyricWikiSpider(CrawlSpider):
    name = "mikeg"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Mike_G",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Mike_G:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
