...
class LyricWikiSpider(CrawlSpider):
    name = "redman"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Redman",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Redman:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
