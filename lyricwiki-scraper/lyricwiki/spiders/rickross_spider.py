...
class LyricWikiSpider(CrawlSpider):
    name = "rickross"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Rick_Ross",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Rick_Ross:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
