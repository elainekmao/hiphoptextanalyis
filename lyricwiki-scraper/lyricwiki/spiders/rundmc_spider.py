...
class LyricWikiSpider(CrawlSpider):
    name = "rundmc"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Run-D.M.C.",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Run-D.M.C.:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
