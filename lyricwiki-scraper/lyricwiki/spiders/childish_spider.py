...
class LyricWikiSpider(CrawlSpider):
    name = "childishgambino"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Childish_Gambino",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Childish_Gambino:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
