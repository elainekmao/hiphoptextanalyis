...
class LyricWikiSpider(CrawlSpider):
    name = "mchammer"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/MC_Hammer",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/MC_Hammer:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
