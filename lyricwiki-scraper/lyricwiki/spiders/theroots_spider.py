...
class LyricWikiSpider(CrawlSpider):
    name = "theroots"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/The_Roots",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/The_Roots:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
