...
class LyricWikiSpider(CrawlSpider):
    name = "mobbdeep"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Mobb_Deep",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Mobb_Deep:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
