...
class LyricWikiSpider(CrawlSpider):
    name = "bonethugs"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Bone_Thugs-N-Harmony",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Bone_Thugs-N-Harmony:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
