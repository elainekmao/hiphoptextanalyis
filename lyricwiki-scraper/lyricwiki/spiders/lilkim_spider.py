...
class LyricWikiSpider(CrawlSpider):
    name = "lilkim"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Lil'_Kim",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Lil.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
