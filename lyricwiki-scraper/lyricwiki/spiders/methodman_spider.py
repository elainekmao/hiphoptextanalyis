...
class LyricWikiSpider(CrawlSpider):
    name = "methodman"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Method_Man",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Method_Man:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
