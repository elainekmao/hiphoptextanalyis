...
class LyricWikiSpider(CrawlSpider):
    name = "jdilla"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/J_Dilla",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/J_Dilla:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
