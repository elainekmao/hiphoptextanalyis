...
class LyricWikiSpider(CrawlSpider):
    name = "junglebrothers"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Jungle_Brothers",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Jungle_Brothers:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
