...
class LyricWikiSpider(CrawlSpider):
    name = "tylerthecreator"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Tyler,_The_Creator",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Tyler.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
