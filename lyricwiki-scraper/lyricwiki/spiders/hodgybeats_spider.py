...
class LyricWikiSpider(CrawlSpider):
    name = "hodgybeats"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Hodgy_Beats",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Hodgy_Beats:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
