...
class LyricWikiSpider(CrawlSpider):
    name = "blackalicious"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Blackalicious",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Blackalicious:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
