...
class LyricWikiSpider(CrawlSpider):
    name = "toneloc"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Tone-Loc",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Tone-L.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
