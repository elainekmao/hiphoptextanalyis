...
class LyricWikiSpider(CrawlSpider):
    name = "skee-lo"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Skee-Lo",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Skee-Lo:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
