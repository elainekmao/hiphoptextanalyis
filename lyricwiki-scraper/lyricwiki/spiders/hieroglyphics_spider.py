...
class LyricWikiSpider(CrawlSpider):
    name = "hieroglyphics"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Hieroglyphics",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Hieroglyphics:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
