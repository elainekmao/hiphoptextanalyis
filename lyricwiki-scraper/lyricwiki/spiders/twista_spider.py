...
class LyricWikiSpider(CrawlSpider):
    name = "twista"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Twista",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Twista:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
