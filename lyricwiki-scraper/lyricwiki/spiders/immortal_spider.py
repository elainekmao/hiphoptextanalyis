...
class LyricWikiSpider(CrawlSpider):
    name = "immortaltechnique"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Immortal_Technique",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Immortal_Technique:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
