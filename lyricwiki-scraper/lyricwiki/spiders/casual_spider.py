...
class LyricWikiSpider(CrawlSpider):
    name = "casual"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Casual",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Casual:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
