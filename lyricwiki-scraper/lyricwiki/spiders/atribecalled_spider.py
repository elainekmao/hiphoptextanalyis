...
class LyricWikiSpider(CrawlSpider):
    name = "atribecalled"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/A_Tribe_Called_Quest",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/A_Tribe_Called_Quest:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
