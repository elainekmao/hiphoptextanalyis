...
class LyricWikiSpider(CrawlSpider):
    name = "kurtisblow"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Kurtis_Blow",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Kurtis_Blow:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
