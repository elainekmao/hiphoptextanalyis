...
class LyricWikiSpider(CrawlSpider):
    name = "eminem"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Eminem",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Eminem:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
