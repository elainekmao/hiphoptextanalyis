...
class LyricWikiSpider(CrawlSpider):
    name = "rza"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/The_RZA",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/The_RZA:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
