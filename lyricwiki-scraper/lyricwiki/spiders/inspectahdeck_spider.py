...
class LyricWikiSpider(CrawlSpider):
    name = "inspectahdeck"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Inspectah_Deck",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Inspectah_Deck:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
