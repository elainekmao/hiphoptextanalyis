...
class LyricWikiSpider(CrawlSpider):
    name = "meekmill"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Meek_Mill",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Meek_Mill:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
