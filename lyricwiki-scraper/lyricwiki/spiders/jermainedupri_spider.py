...
class LyricWikiSpider(CrawlSpider):
    name = "jermainedupri"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Jermaine_Dupri",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Jermaine_Dupri:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
