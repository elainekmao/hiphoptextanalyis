...
class LyricWikiSpider(CrawlSpider):
    name = "cypresshill"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Cypress_Hill",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Cypress_Hill:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
