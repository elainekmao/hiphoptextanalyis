...
class LyricWikiSpider(CrawlSpider):
    name = "nickiminaj"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Nicki_Minaj",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Nicki_Minaj.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
