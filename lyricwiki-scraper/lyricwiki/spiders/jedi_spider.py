...
class LyricWikiSpider(CrawlSpider):
    name = "jedi"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Jedi_Mind_Tricks",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Jedi_Mind_Tricks:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
