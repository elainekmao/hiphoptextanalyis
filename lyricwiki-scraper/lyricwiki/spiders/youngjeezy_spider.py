...
class LyricWikiSpider(CrawlSpider):
    name = "youngjeezy"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Young_Jeezy",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Young_Jeezy:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
