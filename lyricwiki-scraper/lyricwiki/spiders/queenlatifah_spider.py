...
class LyricWikiSpider(CrawlSpider):
    name = "queenlatifah"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Queen_Latifah",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Queen_Latifah:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
