...
class LyricWikiSpider(CrawlSpider):
    name = "bigdaddykane"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Big_Daddy_Kane",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Big_Daddy_Kane:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
