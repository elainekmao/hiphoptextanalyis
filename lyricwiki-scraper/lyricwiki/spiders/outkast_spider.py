...
class LyricWikiSpider(CrawlSpider):
    name = "outkast"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/OutKast",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/OutKast:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
