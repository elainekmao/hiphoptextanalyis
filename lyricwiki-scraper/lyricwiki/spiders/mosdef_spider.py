...
class LyricWikiSpider(CrawlSpider):
    name = "mosdef"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Mos_Def",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Mos_Def:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
