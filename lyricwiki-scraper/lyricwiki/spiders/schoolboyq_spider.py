...
class LyricWikiSpider(CrawlSpider):
    name = "schoolboyq"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Schoolboy_Q",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/ScHoolboy.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
