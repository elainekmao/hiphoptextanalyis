...
class LyricWikiSpider(CrawlSpider):
    name = "bigpun"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Big_Pun",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Big_Pun.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
