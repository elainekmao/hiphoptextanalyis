...
class LyricWikiSpider(CrawlSpider):
    name = "scarface"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Scarface",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Scarface:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
