...
class LyricWikiSpider(CrawlSpider):
    name = "icecube"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Ice_Cube",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Ice_Cube:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
