...
class LyricWikiSpider(CrawlSpider):
    name = "kendricklamar"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Kendrick_Lamar",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Kendrick_Lamar:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
