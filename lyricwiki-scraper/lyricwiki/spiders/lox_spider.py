...
class LyricWikiSpider(CrawlSpider):
    name = "lox"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/The_Lox",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/The_LOX:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
