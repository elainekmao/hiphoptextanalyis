...
class LyricWikiSpider(CrawlSpider):
    name = "thepharcyde"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/The_Pharcyde",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/The_Pharcyde:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
