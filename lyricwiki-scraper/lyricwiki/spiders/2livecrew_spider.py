...
class LyricWikiSpider(CrawlSpider):
    name = "2livecrew"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/2_Live_Crew",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/2_Live_Crew:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
