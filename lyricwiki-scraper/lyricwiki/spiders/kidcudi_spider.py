...
class LyricWikiSpider(CrawlSpider):
    name = "kidcudi"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Kid_Cudi",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Kid_Cudi:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
