...
class LyricWikiSpider(CrawlSpider):
    name = "jadakiss"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Jadakiss",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Jadakiss:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
