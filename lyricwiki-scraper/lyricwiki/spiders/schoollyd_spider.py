...
class LyricWikiSpider(CrawlSpider):
    name = "schoollyd"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Schoolly_D",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Schoolly_D:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
