...
class LyricWikiSpider(CrawlSpider):
    name = "blackmoon"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Black_Moon",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Black_Moon:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
