...
class LyricWikiSpider(CrawlSpider):
    name = "blacksheep"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Black_Sheep",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Black_Sheep.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
