...
class LyricWikiSpider(CrawlSpider):
    name = "brandnubian"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Brand_Nubian",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Brand_Nubian:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
