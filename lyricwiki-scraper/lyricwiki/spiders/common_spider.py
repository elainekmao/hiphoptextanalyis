...
class LyricWikiSpider(CrawlSpider):
    name = "common"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Common",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Common:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
