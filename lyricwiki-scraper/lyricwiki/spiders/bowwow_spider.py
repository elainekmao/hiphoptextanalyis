...
class LyricWikiSpider(CrawlSpider):
    name = "bowwow"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Bow_Wow",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Bow_Wow:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
