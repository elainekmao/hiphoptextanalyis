...
class LyricWikiSpider(CrawlSpider):
    name = "mcren"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/MC_Ren",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/MC_Ren:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
