...
class LyricWikiSpider(CrawlSpider):
    name = "wizkhalifa"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Wiz_Khalifa",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Wiz_Khalifa:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
