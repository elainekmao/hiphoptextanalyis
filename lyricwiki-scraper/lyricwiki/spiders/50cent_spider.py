...
class LyricWikiSpider(CrawlSpider):
    name = "50cent"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/50_Cent",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/50_Cent:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
