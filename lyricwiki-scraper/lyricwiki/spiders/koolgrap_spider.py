...
class LyricWikiSpider(CrawlSpider):
    name = "koolgrap"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Kool_G._Rap",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Kool_G_Rap:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
