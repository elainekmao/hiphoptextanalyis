...
class LyricWikiSpider(CrawlSpider):
    name = "willsmith"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Will_Smith",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Will_Smith:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
