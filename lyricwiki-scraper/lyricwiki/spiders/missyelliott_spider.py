...
class LyricWikiSpider(CrawlSpider):
    name = "missyelliott"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Missy_Elliott",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Missy_Elliott:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
