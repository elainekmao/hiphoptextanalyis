...
class LyricWikiSpider(CrawlSpider):
    name = "grandmasterflash5"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Grandmaster_Flash_And_The_Furious_Five",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Grandmaster_Flash_And_The_Furious_Five:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
