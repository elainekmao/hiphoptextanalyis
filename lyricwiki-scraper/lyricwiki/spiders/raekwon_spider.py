...
class LyricWikiSpider(CrawlSpider):
    name = "raekwon"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Raekwon",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Raekwon:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
