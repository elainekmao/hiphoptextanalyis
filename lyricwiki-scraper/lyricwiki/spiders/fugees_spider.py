...
class LyricWikiSpider(CrawlSpider):
    name = "fugees"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Fugees",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Fugees:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
