...
class LyricWikiSpider(CrawlSpider):
    name = "busta"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Busta_Rhymes",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Busta_Rhymes:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
