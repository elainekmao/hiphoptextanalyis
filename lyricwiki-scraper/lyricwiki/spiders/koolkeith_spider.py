...
class LyricWikiSpider(CrawlSpider):
    name = "koolkeith"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Kool_Keith",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Kool_Keith.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
