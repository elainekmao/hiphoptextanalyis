...
class LyricWikiSpider(CrawlSpider):
    name = "diddy"                                         #CHANGE NAME
    allowed_domains = ["lyrics.wikia.com"]
    start_urls = [
        "http://lyrics.wikia.com/Diddy",     #CHANGE URL
    ]
    rules = (                                               #CHANGE REGEX
        Rule(SgmlLinkExtractor(allow=('/Puff_Daddy:.*|/P._Diddy:.*|/Diddy:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
