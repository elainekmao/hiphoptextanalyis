...
class LyricWikiSpider(CrawlSpider):
    ...
    start_urls = [
        "http://lyrics.wikia.com/2Pac",
    ]
    rules = (
        Rule(SgmlLinkExtractor(allow=('/2Pac:.*',),restrict_xpaths=('//ol/li',)), callback='parse_item', follow=True),
        )
    ...
    #def parse(self, response):
        ...
        #for song in songs:
            #title = song.xpath('b/a/text()').extract()
            #link = song.xpath('b/a/@href').extract()
            #yield LyricWikiItem(title = title, artist = '2Pac')
